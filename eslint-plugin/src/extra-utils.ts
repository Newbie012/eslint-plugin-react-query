...
export const ExtraUtils = {
  ...
  isPropertyWithIdentifierKey(node: TSESTree.Node, key: string): node is TSESTree.Property {
    return (
      ExtraUtils.isProperty(node) && ExtraUtils.isIdentifier(node.key) && node.key.name === key
    );
  },
  findPropertyWithIdentifierKey(
    ...
    return properties.find((x) => ExtraUtils.isPropertyWithIdentifierKey(x, key)) as
      | TSESTree.Property
      | undefined;
  },
  getIdentifiersFromArrayExpression(node: TSESTree.ArrayExpression): TSESTree.Identifier[] {
