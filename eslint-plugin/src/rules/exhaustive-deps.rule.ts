...
const createRule = ESLintUtils.RuleCreator(
  () => "https://github.com/Newbie012/eslint-plugin-react-query"
);
...
const defaultRuleOptions = { whitelist: ["api", "fetch"] };
...
export const exhaustiveDepsRule = createRule<[RuleOptions], RuleMessage>({
  ...
  create(context) {
    return {
      CallExpression(node) {
        if (ExtraUtils.isIdentifier(node.callee) && node.callee.name === "useQuery") {
...
function runCheck(params: { node: TSESTree.CallExpression; context: RuleContext }) {
  ...
  const queryKey = ExtraUtils.findPropertyWithIdentifierKey(queryOptions.properties, "queryKey");
  const queryFn = ExtraUtils.findPropertyWithIdentifierKey(queryOptions.properties, "queryFn");
  ...
  const queryKeyDeps = ExtraUtils.getIdentifiersFromArrayExpression(queryKeyValue);
  ...
  if (missingRefs.length > 0) {
    context.report({
      ...
      fix(fixer) {
        const queryKeyDeps = queryKeyValue.elements.filter(
          (element) => ExtraUtils.isIdentifier(element) || ExtraUtils.isLiteral(element)
        );
        const newQueryKeyDeps = [...queryKeyDeps, ...missingRefs];
